...
class DDPGAgent(Agent):
    def __init__(self, env, network, optimizer, gamma, batch_size, replay_buffer_size, inc_update_factor):
        self.env = env
        self.agent = DDPG(network, optimizer, gamma, batch_size, replay_buffer_size, inc_update_factor)
    ...
    def train(self, n_episodes, max_steps=300):
        ...
        for episode in range(1, n_episodes + 1):
            ...
            while cur_step <= max_steps and not done:
                ...
                action = self.agent.select_action(state)
                noise = torch.random.normal(0, 1, action.shape)
                action = torch.clamp(action + noise, self.env.action_space.low, self.env.action_space.high)
